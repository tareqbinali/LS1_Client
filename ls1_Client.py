...
from constants import app_exe_path, version_no, release_date, icon_file , style_file
...
from PySide6.QtWidgets import QMainWindow, QApplication, QWidget, QPushButton, QToolBar,QGroupBox, QHBoxLayout, QVBoxLayout,\
     QGridLayout, QListWidget, QListWidgetItem, QTableWidget, QTableWidgetItem,QScrollArea,\
     QMessageBox, QFileDialog, QProgressBar, QDialog, QDialogButtonBox, QTextEdit, QLabel,\
     QInputDialog, QMenu,  QCalendarWidget, QHeaderView,  QStackedLayout, \
     QGraphicsView, QGraphicsScene, QGraphicsPixmapItem, QGraphicsPathItem, QSizePolicy
     

from PySide6.QtCore import QSize, QThread, Signal, Qt, QDate, QTimer, QObject, Slot, QUrl, QCoreApplication, QPoint, QPointF, QFile, QTextStream
from PySide6.QtGui import QAction, QIcon, QPixmap, QFont, QKeyEvent,  QPainter, QColor, QPen, QPainterPath
...
class AudioPlayer2(QObject):
    ...
    def __init__(self):
        ...
        # Force the output to stereo (2 channels)
        self.audio_output.setVolume(1.0)  # Ensure volume is not muted
        self.player.setAudioOutput(self.audio_output)
    ...
    def on_error(self, error, error_string):
        print(f"Error occurred: {error_string}, Error Code: {error}")
        print(f"Media error details: {self.player.errorString()}")      
...
class mainUI(QMainWindow):
        ...
        if self.creat_spec_images:
            ...
            if self.SpecFileURL:
                for index, spec_url in enumerate(self.SpecFileURL):
                    ...
                    graphics_view.customContextMenuRequested.connect(self.show_graphics_menu)
    ...
    def show_graphics_menu(self, pos: QPoint):
        ...
        open_plot_action = menu.addAction("reserve")
    ...
    def on_graphics_view_clicked(self, index):
        print(f"Graphics view at index {index} clicked")
    ...
    def play_plot(self, sender_view):
        ...
        print(f"wav file  : {url} ...")

        # self.audio_player.play(url)

        # self.audio_player = AudioPlayer()
        # self.audio_player.load_audio(url)  # Load the audio from URL
        # self.audio_player.show()
    ...
    def revisit_site(self, index):
        print(f"Copy clicked for plot {index}")
        self.listRevisit.addItem(self.RecFilename[index])
    def open_plot(self, index):
        print(f"Process clicked for plot {index}")
   ...
   #Statusbar
    ...
    def saveFile(self):
        ...
        header_file=os.path.join(Folder, 'headers.json')
        saveToCSV(self.data, filename=file_name, header=self.headers, confirm=True)
    ...
    def exportJobList(self):
        try:
            ...
            self.updateLogBox(f'error : {e}')
            msgBox(f'{e}', "error", 'error')
...
# ====================================================================================== 
    ...
    def play_audio(self, audio_url, index=None):
        # # Load and play the audio file using pygame.mixer
        # self.current_audio_file = self.download_audio(audio_url)
        # if self.current_audio_file:
        #     pygame.mixer.music.load(self.current_audio_file)
        #     pygame.mixer.music.play()
        #     # Create a timer to update the waveform plot
        #     self.timer = QTimer(self)
        #     self.timer.timeout.connect(self.update_waveform_plot)
        #     self.timer.start(100)  # Update every 100 milliseconds
        #     self.current_waveform_plot = self.audio_waveform_plots[index]
    ...
    # def read(self):
    #     pass
    def readTable(self, selected_item=None, dataset_id='LS1_Data', tablename='LS1_RawWavURL'):
        self.creat_spec_images=True
        ...
        if selected_item:
            ...
            [self.data,self.headers]=self.gcp.read(dataset=dataset_id, table=tablename, condition_string=selected_item) 
            self.Data=self.data
            self.populateHeaders(selected_item)

    
    def populateHeaders(self, selected_item=None):
    ...
    def createMenu(self): 
        ...
        export_joblist_action=file_menu.addAction("Export job list")
        ...
        update_action=self.help_menu.addAction("checkUpdate")
        ...
        test_a_function_action=self.help_menu.addAction("test a function")
    ...
    def createToolbar(self):
        ...
        # # Add an empty QToolButton as a spacer
        # spacer = QToolButton(self.toolbar)
        # spacer.setText("")  # Empty text for the button
        # spacer.setIconSize(QSize(0, 0))  # No icon
        # spacer.setStyleSheet("border: none;")  # Remove border to make it invisible
        # spacer.setAutoRaise(True)  # Ensure it doesn't show as a button
        ...
        # shadow_effect.setOffset(2, 2)  # Offset of the shadow
        # shadow_effect.setBlurRadius(8)  # Blur radius of the shadow
        # shadow_effect.setColor(Qt.black)  # Shadow color

        
        # Button 
        self.btn_Connect=QPushButton("SEARCH")
        # self.btn_Connect.setGraphicsEffect(shadow_effect)
        self.btn_Connect.clicked.connect(lambda : self.readTable(selected_item=None))
        ...
        # self.btn_Stop.clicked.connect(self.stopRun)
        ...
        reload_action=QAction(QIcon("refresh.png"), "reload table", self)
        reload_action.setStatusTip("reload table")
        reload_action.triggered.connect(self.reloadTable)
        ...
        self.btn_SpecView=QPushButton("SPEC View")
        self.btn_SpecView.clicked.connect(self.populateSpecImages)
        self.btn_SpecView.setVisible(True)
        self.toolbar.addWidget(self.btn_SpecView)
        self.toolbar.addSeparator()
        ...
        self.btn_TableView=QPushButton("Table View")
    ...
    def createWidgets(self):
        ...
        self.btn_SpecView=QPushButton("Spec View")
        self.btn_SpecView.setFont(QFont('Times',14))
        self.btn_SpecView.clicked.connect(lambda : self.populateSpecImages(test_model=True))
